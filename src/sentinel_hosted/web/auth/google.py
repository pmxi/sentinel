...
from sentinel_hosted.config import HostedSettings
...
class GoogleOAuthIdentity(IdentityProvider):
    ...
    def init_app(self, app: Flask) -> None:
        if not HostedSettings.SESSION_SECRET:
            raise RuntimeError(
                "SESSION_SECRET not configured for the hosted runtime."
            )
        if not HostedSettings.GOOGLE_CLIENT_ID or not HostedSettings.GOOGLE_CLIENT_SECRET:
            raise RuntimeError(
                "GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET not configured for the hosted runtime."
            )

        app.secret_key = HostedSettings.SESSION_SECRET

        oauth = OAuth(app)
        oauth.register(
            name="google",
            client_id=HostedSettings.GOOGLE_CLIENT_ID,
            client_secret=HostedSettings.GOOGLE_CLIENT_SECRET,
        ...
        def auth_google_callback():
            ...
            db = HostedDatabase(self.db_path)
            try:
