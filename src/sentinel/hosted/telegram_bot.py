...
from sentinel.hosted.config import settings
...
class TelegramBotListener:
    ...
    def _tick(self, db: HostedDatabase) -> None:
    ...
    def _handle_update(self, db: HostedDatabase, update: Dict[str, Any]) -> None:
    ...
    def _handle_start(self, db: HostedDatabase, chat_id: int, token: str) -> None:
        if not token:
            self._reply(
                chat_id,
                "👋 Sentinel here.\n\n"
